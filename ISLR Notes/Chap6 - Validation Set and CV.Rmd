---
title: "Chap 6 - Validation Approach"
author: "Me"
date: "8/11/2020"
...
```
```{r}
tidy(regfit_best_train) %>%
  gather(key = 'measure', value = 'value', r.squared, adj.r.squared, BIC, mallows_cp) %>%
  group_by(measure)%>%
...
```{r}
library(patchwork)
plot(reg_best, scale = 'bic')/ plot(reg_best, scale = 'adjr2')
```
...
reg_fit = regsubsets(mpg ~ ., data = auto)
summary(reg_fit)
```

```{r}
min_max_pred = tidy(reg_fit) %>%
...
model3 = glm(mpg ~ weight + year + origin, data = train)
model6 = glm(mpg ~ cylinders + displacement + horsepower + weight + year + origin, data = train)

test %>%
  mutate(pred = predict(model3, test)) %>%
  summarise(error = mean((pred-mpg)^2))

test %>%
  mutate(pred = predict(model6, test)) %>%
  summarise(error = mean((pred-mpg)^2))

model_all = glm(mpg~ ., data = train)

test %>%
  mutate(pred = predict(model_all, test)) %>%
  summarise(error = mean((pred-mpg)^2))
```

```{r}
library(boot)
model3 = glm(mpg ~ weight + year + origin, data = auto)
cv_error = cv.glm(auto, model3)
cv_error$delta

model6 = glm(mpg ~ cylinders + displacement + horsepower + weight + year + origin, data = auto)
cv_error = cv.glm(auto, model6)
cv_error$delta

model_all = glm(mpg~ ., data = auto)
cv_error = cv.glm(auto, model_all)
cv_error$delta
