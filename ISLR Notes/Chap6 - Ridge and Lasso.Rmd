---
title: "Chapter 6 - Ridge Regression"
author: "Me"
date: "8/11/2020"
...
```{r}
tidy(ridge_mod) %>%
  ggplot(aes(step, estimate, color = term))+
  geom_line()
```
...
Ang ginagawa ng predict dito ay kinocompute yung coefficients for new value of lambda. The argument type = 'coefficients' was needed to tell the program that the new data is through input
```{r}
...
data.frame(
  ...
  summarise(error = mean((X1 - test)^2))
```
...
At high lambda, all coefficients are near 0
```{r}
...
OLS and ridge regression with lambda = 0 will yield the same result
```{r}
ridge_pred = predict(ridge_mod, s = 0.01, newx = x_test, exact = T)
mean((ridge_pred - y_test)^2)
```
```{r}
...
```{r}
predict(ridge_mod, s = 0.01, exact = T, type="coefficients")[1:20,]
```
...
Basically lambda with the lowest estimated error
```{r}
tidy(cv.out) %>%
  filter(estimate == min(estimate))
```

This chunk tries to mimic the plot output of cv.out
```{r}
tidy(cv.out) %>%
...
Predict the coefficients using the best lambda
First we need to create a ridge model using all data and then predict the coefficients with the best lambda
```{r}
...
```{r}
#Create a lasso model using training data
...
First, use cross validation to simulate outputs of different values of lambda and then plot MSE as a function of lambda
```{r}
set.seed(1)
...
Estimate the coefficients for the whole data using the identified best lambda
```{r}
...
ridge_mod_all = glmnet(x, y, alpha = 0, lambda = grid)
plot(ridge_mod_all)
```
```{r}
library(broom)
...
Identify the best lambda using train
```{r}
set.seed(1)
...
ridge_pred = predict(ridge_mod, s = min_lambda, newx = x_test)
mean((ridge_pred - y_test)^2)
```
The mean error for using ridge regression using the best lambda is 62.

```{r}
out  = glmnet(x,y, alpha = 0)
predict(out, ridge_mod, s = min_lambda, type = 'coefficients')[1:14,]
```
...
```{r}
test %>%
  mutate(pred = predict(lm, test)) %>%
  summarise(MSE = mean((crim - pred)^2))
```
...
ridge_mod_all = glmnet(x, y, alpha = 1, lambda = grid)
plot(ridge_mod_all)
```
